...
- **Pendiente de decidir con el usuario**: la historia original de Trello también pedía etiquetar los artículos de "Notas Blog", pero esos son contenido compartido de solo lectura (`firestore.rules`: `write: false` para `blogArticles`/`nutritionContent`) — no tiene sentido que cada usuario los edite. Falta definir el enfoque (¿tags por-usuario en una subcolección aparte? ¿se descarta esa parte de la historia?) antes de tocar `content-feed.js`.
